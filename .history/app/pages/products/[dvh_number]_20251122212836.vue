...
import autoTable from 'jspdf-autotable' // ✅ импорт функции отдельно
import fontFile from '@/plugins/static/Roboto-Thin.ttf'
// динамический импорт будет внутри функции generatePDF
...
async function generatePDF() {
  ...
  // 1️⃣ Подключаем шрифт кириллицы
  const fontBuffer = await fetch(fontFile).then(res => res.arrayBuffer())
  doc.addFileToVFS('Roboto-Regular.ttf', fontBuffer)
  doc.addFont('Roboto-Regular.ttf', 'Roboto', 'normal')

  // ждём, пока шрифт будет установлен
  doc.setFont('Roboto')

  // теперь безопасно делать текст и таблицу
  doc.setFontSize(16)
  doc.text('АКТ приема товара', pageWidth / 2, 15, { align: 'center' })
  doc.setFontSize(12)
  doc.text(`DVH Number: ${route.params.dvh_number}`, 14, 25)
  doc.text(
    `Принят оператором СВХ: ${products.value[0]?.operator_who_registered || '-'}`,
    14,
    32
  )
