...
<script setup lang="ts">
  ...
  async function generatePDF() { try { const dvh_number = route.params.dvh_number as string const response = await ProductService.generatePdf(dvh_number) console.log("PDF byteLength:", response.data.byteLength) const pdfBlob = new Blob([response.data], { type: "application/pdf" }) pdfUrl.value = URL.createObjectURL(pdfBlob) pdfDialog.value = true console.log("PDF Size:", response.data.byteLength) } catch (err) { console.error("Ошибка генерации PDF:", err) } }
