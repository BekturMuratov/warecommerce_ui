...
<template>
  <!-- === КНОПКА — ЭТО И ЕСТЬ КОМПОНЕНТ С СНАРУЖИ === -->
  <v-btn color="primary" @click="openModal">
    Загрузить товары через PDF
  </v-btn>

  <!-- === МОДАЛЬНОЕ ОКНО PDF === -->
  <v-dialog v-model="showPdfModal" max-width="2400px" scrollable>
    <v-card>
      <v-card-title class="text-h6">Загрузка товаров через PDF</v-card-title>
      <v-divider />
      ...
      <v-card-text style="padding: 0; height: 80vh;">
        <div class="d-flex" style="height: 100%;">
          ...
          <!-- LEFT: PDF -->
          <div style="flex: 2; border-right: 1px solid #eee; display: flex; flex-direction: column;">
            <div class="pa-3">
              <v-file-input
            ...
            <div style="flex: 1; overflow-y: auto; padding: 10px;">
              ...
              <div v-else class="text-grey text-center mt-5">Загрузите PDF</div>
            </div>
          </div>
          ...
          <div style="flex: 1; padding: 20px; display: flex; flex-direction: column;">
            ...
            <div style="flex: 1; overflow-y: auto;">
              ...
              <div v-else class="text-grey text-center mt-5">
      ...
      <v-card-actions>
        <v-spacer />
        <v-btn variant="tonal" @click="showPdfModal = false">Закрыть</v-btn>
      </v-card-actions>
    </v-card>
  </v-dialog>

  <!-- === МОДАЛКА ДОБАВЛЕНИЯ ТОВАРА === -->
  <v-dialog v-model="showAddModal" max-width="400px">
    <v-card>
